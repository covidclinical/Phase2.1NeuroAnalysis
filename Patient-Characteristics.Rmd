...
This notebook describes the **87,869** hospitalized COVID-19 positive patients that were studied as part of the [Consortium for Clinical Characterization of COVID-19 by EHR](https://covidclinical.net/) Neurology group.
...
```{r}
# need to add total comorbidity counts among adult and pediatric patients in order to properly calculate percents for the combined table
...
### Adult & Pediatric Clinical Kruskall-Wallis
...
comorb_matrix <- function(comorbidity_table, comorbidity) {
...
comorbidity_chi_list <- list()

for(i in top_comorb_adults) {
  
  comorb_chi_results <- comorb_matrix(comorbidity_table = comorb_table_wide, i)
  comorbidity_chi_list[[i]] <- comorb_chi_results
}

comorbidity_chi <- comorbidity_chi_list %>%
  bind_rows() 

chisq_combine_comorb <- chisq_combine %>%
  mutate(X2 = as.numeric(X2),
  df = as.numeric(df)) %>%
  bind_rows(comorbidity_chi) 
```
...
neuro_icds_10 <-
  readxl::read_excel("C:/Users/User/Box Sync/Projects/nu4ce/neuro/Phase2.1NeuroRSummaries/analysis_v2/public-data/2020-02-22_neuro-icd10_CNSvPNS.xlsx", sheet = 2) %>% 
  rename(
    "icd" = `ICD-10`,
...
compute_stratified_neuro_counts <- function(tableone, sorted_sites, is_pediatric = FALSE) {
  ...
  for (i in sorted_sites) {
    ...
    try(
    diag_table_list[[i]] <- tmp_diag %>%
      select(variable, starts_with("n_var")) %>% 
...
compute_stratified_neuro_counts_age <- function(stratified_neuro_counts, tableone) {
  ...
  stratified_neuro_counts <- stratified_neuro_counts %>% 
    filter(grepl("Years", variable),
         ...
         mutate(perc = as.character(round(total_n_var/n_var*100,1)),
...
group.colors <- c(NNC = "#621561", PNS = "#d90429", CNS ="#d6b9b4")

pediatric_plot <- ggplot(stratified_neuro_counts_age_pediatric, 
 ...
 geom_text(
  ...
  facet_grid(pns_cns ~ variable, switch="y", scale = "free_y") +   # Put the y facet strips on the left
  scale_y_discrete("", position="right") +   # Put the y-axis labels on the right
  scale_fill_manual(values = group.colors) + 
  ylab("") + 
  xlab("Number of Patients") + 
  xlim(0, 35) + 
  ggtitle("a. Pediatric Patients") +
  theme(plot.title = element_text(size = 12, face = "bold", hjust = 0))

ggsave("figures/Fig3A_diagnoses_by_age_pediatrics.png", width = 12, height = 7, dpi = 300)
```

```{r fig.height=9, fig.width=15}
adult_plot <- ggplot(stratified_neuro_counts_age_adult, 
 aes(x = total_n_var, y = reorder(description, total_n_var), fill = pns_cns)) +
  geom_bar(stat = "identity") +
 geom_text(
    aes(x = total_n_var, y = description, label = perc), 
  ...
  facet_grid(pns_cns ~ variable, switch="y", scale = "free_y") +   # Put the y facet strips on the left
  scale_y_discrete("", position="right", labels = function(x) str_wrap(x, width = 150)) +   # Put the y-axis labels on the right 
  scale_fill_manual(values = group.colors) + 
  ylab("") + 
  xlab("Number of Patients") + 
  xlim(0, 3500) + 
  ggtitle("b. Adult Patients")  +
  theme(plot.title = element_text(size = 12, face = "bold", hjust = 0))

ggsave("figures/Fig3B_diagnoses_by_age_adult.png", width = 14, height = 12, dpi = 300)
```

**Diagnoses by Age**

```{r fig.height=20, fig.width=15}
grid.arrange(pediatric_plot, adult_plot, ncol=1)

ggsave("figures/Figure3.Diagnoses_by_age.pdf", grid.arrange(pediatric_plot, adult_plot, ncol=1), width = 18, height = 15, dpi = 300)
```

**Diagnoses by Adult & < 18**
...
total_ped_age_count = n_0_2 + n_6_11 + n_12_17
...
ggsave("figures/Fig3_diagnoses_by_age_adult_ped_18group.png", adult_ped_combined_plot, width = 40, height = 18, dpi = 300)
```
...
n_death_adult = get_table_n(df = tableone_adult, var = "Deceased")
n_severe_adult = get_table_n(df = tableone_adult, var = "Severe")

n_death_pediatric = get_table_n(df = tableone_pediatric, var = "Deceased")
n_severe_pediatric = get_table_n(df = tableone_pediatric, var = "Severe")
...
print(paste(both_counts, "(", round(both_counts/(tableone_combine$N[1]+both_counts)*100,2), "%)")) 
```
...
calculate_rr <- function(is_pediatric = FALSE) {
  ...
  ggsave(paste0("figures/Figure4.relative_risk_", pop, ".png"), grid.arrange(cns_rr_plot, pns_rr_plot, ncol=2), width = 12, height = 8, dpi = 300)
...
ggsave("figures/SuppFig_pca_deviance.png", pca_plot, height = 4, width = 6)
```
