...
forester <- function (left_side_data, estimate, ci_low, ci_high, right_side_data = NULL,
          ...
          file_path = here::here("forester_plot.png"), dpi = 300,
          ...
          center_ggplot = NULL, set_png_height = NULL)
{
  theme <- gridExtra::ttheme_minimal(core = list(fg_params = list(hjust = 0,
                                                                  ...
                                                                  cex = 1), bg_params = list(fill = c(rep(c(stripe_colour,"white"),
                                                                                                          length.out = nrow(left_side_data)), "white", "white", "white"))),
                                     colhead = list(fg_params = list(hjust = 0, x = 0.05, fontfamily = font_family),
  ...
  if (is.null(right_side_data)) {
    tdata <- gdata
    tdata <- dplyr::mutate_all(tdata, ~sprintf(., fmt = paste0("%#.", estimate_precision, "f")))
    tdata[tdata == "NA"] <- " "
    right_side_data <- data.frame(Estimate = ifelse(tdata$estimate ==  " ", " ", paste0(tdata$estimate,
                                                                                        " (", tdata$ci_low,
                                                                                        " to ", tdata$ci_high, ")")))
  ...
  h_adj <- dplyr::case_when(font_family == "mono" ~ 0.2,
                            font_family == "serif" ~ 0.43,
                            font_family == "sans" ~ 0.37,
                            TRUE ~ 0)
  h_adj <- nudge_y + h_adj
  slope_adj <- dplyr::case_when(font_family == "mono" ~ -0.175,
                                font_family == "serif" ~ -0.19,
                                font_family == "sans" ~ -0.16,
                                TRUE ~ 0)
  ...
  if (!is.null(xlim)) {
    ...
    if (ra) {
      right_arrows <- dplyr::select(dplyr::filter(oob_arrows,
                                                  ci_high > x_high), start = estimate, end = x_high, y = row_num)
    }
    if (la) {
      left_arrows <- dplyr::select(dplyr::filter(oob_arrows,
                                                 ci_low < x_low), start = estimate, end = x_low, y = row_num)
    }
    if (ra && !la) {
      g_oob <- right_arrows
    }else if (!ra && la) {
  ...
  center <- ggplot2::ggplot() +
    ggplot2::geom_point(data = gdata,
                        ggplot2::aes(y = row_num, x = estimate, size = sizes,
                                     shape = shape, colour = as.factor(shape)), na.rm = TRUE) +
    ggplot2::geom_errorbarh(data = gdata,
    ...
    ggplot2::theme(axis.title.y = ggplot2::element_blank(),
                   ...
                   axis.line.y = ggplot2::element_blank(), axis.ticks.length.x = grid::unit(0.07, "in"),
    ...
    ggplot2::xlab("") #+
    #scale_x_break(c(2.5,4.5), scale = 0.3)
  ...
  if (!is.null(set_png_height)) {
    png_height = set_png_height
  }
  ...
  if (is.null(add_plot)) {
    ...
    table_final <- mono_column(gridExtra::tableGrob(tdata_print,
                                                    theme = theme, rows = NULL),ncol(left_side_data) + 1)
